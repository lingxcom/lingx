...
function getItems(list){
	...
	for(var i=0;i<list.length;i++){
		...
		str=str+"<li><a href='javascript:;' onclick='addTab(\""+list[i].itemId+"\",\""+list[i].text+"\",\""+list[i].uri+"\")'>"+list[i].text+"</a></li>";
		if(list[i].menu){
...
Ext.onReady(function(){
	Lingx.post("d",{c:"menu"},function(json){
		...
		for(var i=0;i<json.length;i++){
			menus.push({title:json[i].text,html:"<ul class='leftmenu'>"+getItems(json[i].menu)+"</ul>",iconCls:"Resultsetnext",bodyStyle:"padding:0px;"});
		}
